...
exports.hyperion = function(spec) {
    ...
    wss.on('connection', (ws) => {
        ...
        ws.on('message', (message) => {
            try {
                ...
                if (validMessage.type === 'object-call') {
                    handleObjectCall(ctx, validMessage)
                }
            }
            catch (e) {
                console.log(e)
        ...
        handleMethodResult(ws, 'index' , 0, index)
    })


    function handleObjectCall(ws, msg) {
        var objRecord = objectStore.lookupByName(msg.name)
        if (objRecord === undefined) return

        const node = Multiobserve.findNode(objRecord.object, msg.path)

        if (node !== undefined && typeof node === 'function') {
            const result = node.apply(objRecord.object, [ws].concat(msg.args))
    ...
    function handleMethodResult(ws, name, id, result) {
        const object = objectStore.lookupByObject(result)
        if (object !== undefined) {
